...
>(({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className='flex'>
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn(
                'flex flex-col items-start py-4 font-medium transition-all data-[state=open]:bg-[#FFF4D6] group w-full',
                className
            )}
            {...props}
        >
            <span className='flex-1 w-full mb-4'>{children}</span>
            <div className='absolute w-4 h-4 self-start bottom-0'>
